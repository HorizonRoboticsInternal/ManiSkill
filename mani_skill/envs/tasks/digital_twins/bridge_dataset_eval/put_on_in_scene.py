...
class Hobot2RedCubeScene(Hobot2BridgeEnv):
    ...
    def __init__(
        ...
        xyz_configs = torch.tensor([[-0.125, 0, 0.887529],
                                    [100, 0, 0]])
        quat_configs = torch.tensor([[1, 0, 0, 0],
                                     [1, 0, 0, 0]])
...
class StackGreenCubeOnYellowCubeBakedTexInScene(BaseBridgeEnv):
    ...
    def __init__(
        ...
        # Hardcoding xyz_configs here for now
        xyz_configs = [np.array([[-0.125, 0, 0.887529], [-0.125, 0.1,
                                                         0.887529]])]
        xyz_configs = torch.tensor(np.stack(xyz_configs))
        source_obj_name = "baked_red_cube_3cm"
